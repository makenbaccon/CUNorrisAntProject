Globals [
  Energy      ;; amount of energy of the system
...
to create-food
  if (distancexy (0.85 * max-pxcor) 0) < 6
    [ set food-source-number 1 ]
    ;; setup food source two on the lower-left
    if (distancexy (-0.6 * max-pxcor) (-0.6 * max-pycor)) < 5
    [ set food-source-number 2 ]
    ;; setup food source three on the upper-left
    if (distancexy (-0.7 * max-pxcor) (0.7 * max-pycor)) < 6
...
to recolor-patch  ;; patch procedure
  ...
  [ ifelse food > 0       ;; Color food
    ...
    [ ifelse chemical <= Chem-Low-Threshold             
       [ set pcolor 65 ]
       [ ifelse chemical >= Chem-Upper-Threshold
         [ set pcolor white ]
;;      if chemical <= Chem-Low-Threshold and chemical >= Chem-Upper-Threshold
         [ set pcolor scale-color green chemical Chem-Low-Threshold Chem-Upper-Threshold ]
    ] ] ]
end
...
to Measure-Energy
  set Energy (count turtles * 1 + count turtles with [pcolor = black] * 2 + chemical)
end

to Add-ants
  if (distancexy 0 0) < 6
      [   set Leave (Leave + chemical) / 2 ]
  if leave < 30 [ set window 0 ] 
  if leave > 60 [ set window 1 ] 
  
  if Ticks Mod 10 = 0 and count turtles < 1000 and window = 0
     [ if Leave > 30 and Leave < 60
       [ hatch-forage 1 [ set color red set hungry 0] ]]
end 
...
to do-plotting
  ...
  set-current-plot "Number of Ants"
  set-current-plot-pen "forage"
  plotxy ticks count forage ;;of turtles with [pcolor = red]

  set-current-plot "Energy" ;; plot energy vs time
  plotxy ticks energy
  
  set-current-plot "Leave"
