...
to create-food
  if (distancexy (0.85 * max-pxcor) 0) < 6
    ...
    if (distancexy (-0.6 * max-pxcor) (-0.6 * max-pycor)) < 6
...
to go  ;; forever button
  ...
  measure-system
;  set power (count turtles * 1 + count turtles with [color = black] * 3 )
...
to return-to-nest  ;; turtle procedure
  ...
  [ ;; drop food and head out again
    ...
    rt 180 ]
  [ If breed = patroller 
    [set chemical chemical + 100]  ;; drop some chemical
    If breed = forager
    [set chemical chemical + 25]  ;; drop some chemical
...
380
Low Chem Area
(count patches with [chemical > chem-low-threshold]) / (count patches)
3
1
9
...
380
Upper Chem Area
(count patches with [chemical > chem-upper-threshold]) / (count patches)
3
1
9
