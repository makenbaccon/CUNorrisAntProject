...
patches-own [nest-config]  ;;Property to setup the gradient of patches to get home
...
to setup-ant-hill
  ask patch 0 0 [set pcolor blue]                                ;;the anthill is blue
...
to find-food
  ...
  if pcolor = green[              ;;if the ant is on food pick it up
    set food 1
    set color green
    set pcolor black
  ]
...
to go-home
  ...
  if (food = 1) and (pcolor = blue)[        ;;values that need to be set if the ant is returning from the outside
  ...
  if (exit = 1) and (pcolor = blue)[        ;;values that need to be set for ants leaving the antpile
...
to interact
  ...
  fd 0.5                                                     ;;step is smaller because in nest
  ...
  if color = green[
    set scent (5 * (e ^ (-0.02 * (ticks - time + 1))))       ;;ants that return have a decay of scent
    
    if scent < 0.05 * 5[                                     ;;if scent is 5% of the original leave the nest
    ...
    if nearest-neighbor != nobody[                           ;;if there is a neigbor around
      ask nearest-neighbor [
        set scent (scent + 5)                                ;;increase the smell of it by original value
      ]
  ...
  if color = white[                                          ;;action that ant takes in nest 
    ...
    if scent > 11[                                           ;;if it breaks the threashold leave nest and find food
      set exit 1                                             ;;set exit flag
