...
patches-own [
  ...
  total-food
  ;; This is the total food that the ants have collected
...
globals[
  ...
  population
  ;; this is the total population of the colony
...
to setup-world
  ...
  ;; Now we need to create the differences in various patches
  ;; first we mark the ant hill at (0,0)
  ask patch 0 0 [set pcolor magenta]
  
  ;; next we set the patches onhill variable to 1 if they are within the nest
  ;; radius
  ask patches [ 
    ifelse ( distancexy 0 0 < nestradius ) 
    [ set onhill? true ]
    [ set onhill? false ]
  ]
...
to create-agents
  ...
  ;; second we will create the nest patrollers which make up 15% of the population
  create-nest-patrollers ceiling (population * 0.015) [
    set color red ;; this sets the color of the nestpatrollers to red
  ]
  
  ;; Now we will create the rest of the inactives
  create-inactives 0.1 * (population - count nest-patrollers ) [
    set color blue
    set hidden? true
    set food-direction [] ;; we create an empty array for the direction as this
                          ;; will be used later when the inactives change jobs
  ]
...
"" ""
PENS
"Foragers" 1.0 0 -14835848 true "" "plot count turtles"
