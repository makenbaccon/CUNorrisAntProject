...
breed [trail-patrollers trail-patroller] ;;forage for food
...
to setup-ant-hill
  ...
  set dish-radius floor (max-pycor * 0.66) ;; or whatever
  ask patches [ set in-dish? false ]
  ask turtle 0 [ask patches in-radius dish-radius [ set in-dish? true ]]

end

to setup-antifood
  repeat ceiling (max-pxcor * max-pycor * (amount-midden / 100))
  [
    ask one-of patches
     [
       set pcolor white
       set midden-amount ((random 3) + 1)
       ]
  ]
...
to setup-food
  ...
  if food-source-number > 0
   [ set foodp 2 ] ;;chagne to random for amount of food per block
...
to nest-patrollers-task  
  ...
  ;;fd 1
   ...
   set patrol-completion (patrol-completion + random 9)
   ...
   [
     ...
     [
       ...
       set color green
       set permenant 1
       set midden-return (midden-return + 1)
     ]
...
to middens-task
  ...
  if hands-full = 1[
    ...
    if not [in-dish?] of patch-ahead 1
    [
      set hands-full 0
      set color green
      set pcolor violet
...
to trail-patrollers-task
    ...
    if midden-return >= floor (population * 0.015 * 0.85)
     [
