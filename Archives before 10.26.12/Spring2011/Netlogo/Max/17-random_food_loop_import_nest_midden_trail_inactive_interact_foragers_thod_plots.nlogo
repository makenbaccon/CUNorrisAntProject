...
to setup-food
  ...
  ask patches [
    ...
    if (distancexy (x2 * location-power2 * max-pxcor) (y2 * max-pycor)) < 2 ;;last number is radius !make scale to world size
    [ set food-source-number 2 ]
    ;; setup food source three on the upper-left
    if (distancexy (x3 * location-power3 * max-pxcor) (y3 * max-pycor)) < 2
